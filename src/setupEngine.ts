...
export function analyzeLong(symbol:string,bars5:Bar[],bars15:Bar[],prev:Bar,now=Date.now()):Setup{
 const base={symbol,timeframe:'5m/15m' as const,price:bars5.at(-1)?.close||0,reasons:[] as string[]};
 if(!bars5.length||!bars15.length)return {...base,status:'LEAVE IT',reasons:['Insufficient 5m/15m candle data.']};
 const cp=cpr(prev);const price=base.price;const reasons:string[]=[];
 ...
 for(let i=0;i<recent.length-2;i++){const a=recent[i],b=recent[i+1],c=recent[i+2];const r=Math.max(a.high,a.low)-Math.min(a.high,a.low);if(c.low>a.high&&bullish(b)&&bullishDisplacement(b,r)){fvg={low:a.high,high:c.low,idx:i+2}}}
 let ob:{low:number;high:number;idx:number}|undefined;
 for(let i=0;i<recent.length-4;i++){const a=recent[i];if(!bullish(a)){for(let j=i+1;j<Math.min(i+5,recent.length);j++){const d=recent[j];const range=d.high-d.low;if(bullishDisplacement(d,range)&&d.close>a.high){ob={low:a.low,high:a.high,idx:j};break}}}}
 ...
 const last=recent.at(-1)!;let selected:any;
 ...
 const entryLow=selected.low,entryHigh=Math.min(selected.high,last.high);const entry=last.close;const stop=selected.low-(selected.high-selected.low)*0.25;
 const priorHigh=prev.high;const swingHigh=Math.max(...recent.slice(0,-1).map(b=>b.high));const targets=[priorHigh,swingHigh].filter(x=>x>entry).sort((a,b)=>a-b);const t1=targets[0],t2=targets.find(x=>x>t1)||undefined;
 ...
 const risk=entry-stop;const rr1=(t1-entry)/risk;const rr2=(t2-entry)/risk;if(risk<=0||rr1<1.5)return {...base,status:'WAIT',cpr:cp,confirmation:{time:new Date(last.t*1000).toISOString(),close:last.close},reasons:[...reasons,'Confirmation exists, but risk/reward is below the minimum 1.5R threshold.']};
